...
<Callout type={"info"}>
  This feature is provided by the `@endexai/blocknote-xl-docx-exporter`. `xl-` packages
...
First, install the `@endexai/blocknote-xl-docx-exporter` and `docx` packages:

```bash
npm install @endexai/blocknote-xl-docx-exporter docx
```
