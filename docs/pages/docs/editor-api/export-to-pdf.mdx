...
<Callout type={"info"}>
  This feature is provided by the `@endexai/blocknote-xl-pdf-exporter`. `xl-` packages
...
First, install the `@endexai/blocknote-xl-pdf-exporter` and `@react-pdf/renderer` packages:

```bash
npm install @endexai/blocknote-xl-pdf-exporter @react-pdf/renderer
```
...
```typescript
import { PDFExporter, pdfDefaultSchemaMappings } from "@endexai/blocknote-xl-pdf-exporter";
import { Text } from "@react-pdf/renderer";
...
const defaultOptions = {
  ...
  colors: COLORS_DEFAULT, // defaults from @endexai/blocknote-core
};
```
