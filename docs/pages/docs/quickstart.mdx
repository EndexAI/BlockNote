...
```console
npm install @endexai/blocknote-core @endexai/blocknote-react @endexai/blocknote-mantine
```
...
We also import `@endexai/blocknote-mantine/style.css` to add default styling for the editor and the `Inter` font that BlockNote exports (optional).
